...
use adk_rs_core::{LlmRequest, LlmResponse, Model};
use adk_rs_error::{Error, ProviderError, Result};

use crate::convert::{parse_response, to_wire};
use crate::stream::from_sse;
...
mod tests {
    ...
    async fn generate_content_happy_path() {
        ...
        .unwrap();
        let req = LlmRequest {
            contents: vec![adk_rs_genai_types::Content::user_text("hi")],
            ..Default::default()
        };
    ...
    async fn stream_endpoint_uses_sse_query() {
        ...
        let stream = g
            ...
            .await
            .unwrap();
        let chunks = crate::stream::collect_stream(stream).await.unwrap();
