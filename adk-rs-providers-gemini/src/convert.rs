...
use adk_rs_core::{LlmRequest, LlmResponse};
use adk_rs_error::Result;
use adk_rs_genai_types::{Content, GenerateContentConfig, GenerateContentResponse};
...
pub(crate) struct WireRequest<'a> {
    ...
    pub tools: &'a [adk_rs_genai_types::Tool],
    #[serde(skip_serializing_if = "Option::is_none", rename = "toolConfig")]
    pub tool_config: Option<&'a adk_rs_genai_types::ToolConfig>,
    ...
    pub safety_settings: &'a [adk_rs_genai_types::SafetySetting],
}
...
pub(crate) struct GenerationConfig<'a> {
    ...
    pub response_schema: Option<&'a adk_rs_genai_types::Schema>,
    #[serde(skip_serializing_if = "Option::is_none", rename = "thinkingConfig")]
    pub thinking_config: Option<&'a adk_rs_genai_types::ThinkingConfig>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub seed: Option<i64>,
...
pub(crate) fn parse_stream_chunk(payload: &str) -> Result<LlmResponse> {
    ...
    if r.finish_reason
        .map(|f| matches!(f, adk_rs_genai_types::FinishReason::Stop))
        .unwrap_or(false)
    {
        // final
...
mod tests {
    use super::*;
    use adk_rs_core::LlmRequest;
    use adk_rs_genai_types::{Content, FunctionCall, GenerateContentConfig, Part, Role, Schema, Tool};
    use serde_json::json;
    ...
    fn to_wire_emits_function_decls_in_tools() {
        ...
        req.config.tools.push(Tool::FunctionDeclarations(vec![
            adk_rs_genai_types::FunctionDeclaration::new("f", "do f")
