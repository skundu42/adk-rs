...
use adk_rs_agents::BaseAgent;
use adk_rs_runner::Runner;
use adk_rs_services_mem::InMemorySessionService;
use adk_rs_telemetry::{LogFormat, TelemetryConfig};
...
impl App {
    ...
    pub fn run(self) -> adk_rs_error::Result<()> {
        let cli = Cli::parse();
        adk_rs_telemetry::init(TelemetryConfig {
            filter: cli.log,
        ...
        let rt = tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .build()
            .map_err(|e| adk_rs_error::Error::other(format!("tokio: {e}")))?;
        rt.block_on(self.run_async(cli.command))
    }

    /// Async dispatch — useful for tests.
    pub async fn run_async(self, cmd: Command) -> adk_rs_error::Result<()> {
        match cmd {
            Command::Run {
                agent,
                user,
            ...
            Command::Web { bind } => {
                ...
                let state = adk_rs_server::AppState {
                    runners: Arc::new(runners),
                };
                info!("starting dev server on http://{bind}");
                adk_rs_server::serve(bind, state).await
            }
    ...
    fn find_agent(&self, name: &str) -> adk_rs_error::Result<Arc<dyn BaseAgent>> {
        self.agents
            .get(name)
            .cloned()
            .ok_or_else(|| adk_rs_error::Error::not_found(format!("agent {name}")))
    }

    fn build_runner(&self, agent_name: &str) -> adk_rs_error::Result<Runner> {
