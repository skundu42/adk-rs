...
use adk_rs_error::{ProviderError, Result};
use adk_rs_genai_types::{
...
pub(crate) fn to_wire<'a>(req: &'a LlmRequest, model: &'a str) -> WireRequest<'a> {
    let system = req
        ...
        .system_instruction
        .as_ref()
        .map(adk_rs_genai_types::Content::text_concat);
    ...
    let tools: Vec<WireTool> = req
        ...
        .filter_map(|t| match t {
            adk_rs_genai_types::Tool::FunctionDeclarations(decls) => Some(decls),
            _ => None,
        })
...
mod tests {
    use super::*;
    use adk_rs_genai_types::{FunctionCall, Tool};
    use serde_json::json;
    ...
    fn function_decls_are_lowercased_in_input_schema() {
        ...
        req.config.tools.push(Tool::FunctionDeclarations(vec![
            FunctionDeclaration::new("f", "").with_parameters(adk_rs_genai_types::Schema::object()),
        ]));
        let w = serde_json::to_value(to_wire(&req, "x")).unwrap();
