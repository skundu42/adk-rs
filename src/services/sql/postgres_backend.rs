...
impl SessionService for SqlSessionService {
    ...
    /// then writes through to PostgreSQL using a snapshot. The SQL write may
    /// briefly trail the in-memory view; on reload, `events` is authoritative
    /// (each INSERT is independent) while the aggregate `state` is
    /// last-writer-wins.
