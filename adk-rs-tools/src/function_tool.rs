...
mod tests {
    ...
    use adk_rs_core::{InvocationContext, InvocationOrigin, RunConfig};
        ...
        pub(super) fn dummy_invocation() -> InvocationContext {
            ...
            impl adk_rs_core::SessionService for NoopSession {
                async fn create_session(
                    ...
                    app: &str,
                    user: &str,
                    _state: Option<adk_rs_core::State>,
                    id: Option<&str>,
                ) -> adk_rs_error::Result<adk_rs_core::Session> {
                    Ok(adk_rs_core::Session::new(app, user, id.unwrap_or("s")))
                }
                ...
                ) -> adk_rs_error::Result<Option<adk_rs_core::Session>> {
                    Ok(None)
                }
                async fn list_sessions(
                    ...
                    _: &str,
                    _: &str,
                ) -> adk_rs_error::Result<adk_rs_core::ListSessionsResponse> {
                    Ok(adk_rs_core::ListSessionsResponse::default())
                }
                async fn delete_session(&self, _: &str, _: &str, _: &str) -> adk_rs_error::Result<()> {
                    Ok(())
                }
            }
            ...
            InvocationContext {
                ...
                session: Arc::new(Mutex::new(adk_rs_core::Session::new("app", "u", "s"))),
