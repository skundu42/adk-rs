...
/// Usage:
/// ```ignore
/// let cb = adk_rs_core::before_agent_callback!(|ctx| async move {
///     Ok(None)
/// });
/// ```
#[macro_export]
macro_rules! before_agent_callback {
    ($f:expr) => {{
        let f = $f;
        ::std::sync::Arc::new(
            move |ctx: &mut $crate::callback::CallbackContext| -> ::futures::future::BoxFuture<
                '_,
                $crate::Result<::std::option::Option<$crate::types::Content>>,
            > { ::std::boxed::Box::pin((f)(ctx)) },
        ) as $crate::callback::BeforeAgentCallback
    }};
}
