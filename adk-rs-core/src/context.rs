...
use adk_rs_genai_types::Content;

use crate::run_config::RunConfig;
use crate::services::{ArtifactService, CredentialService, MemoryService, SessionService};
...
impl InvocationContext {
    ...
    /// reached.
    pub fn check_and_inc_llm_call(&self) -> adk_rs_error::Result<()> {
        let mut n = self.llm_call_count.lock();
        if let Some(cap) = self.run_config.max_llm_calls {
            if *n >= cap {
                return Err(adk_rs_error::Error::config(format!(
...
impl ToolContext {
    ...
    pub async fn save_artifact(
        &mut self,
        filename: &str,
        part: adk_rs_genai_types::Part,
    ) -> adk_rs_error::Result<u64> {
        let svc = self
            .invocation
            .artifact_service
            .as_ref()
            .ok_or_else(|| adk_rs_error::Error::config("no artifact service configured"))?;
        let key = crate::artifact::ArtifactKey::new(
    ...
    pub async fn load_artifact(
        ...
        filename: &str,
        version: Option<u64>,
    ) -> adk_rs_error::Result<Option<adk_rs_genai_types::Part>> {
        let svc = self
            .invocation
            .artifact_service
            .as_ref()
