...
let m: Arc<dyn Model> = registry.get("gemini-2.5-pro").expect("glob match");`,
    ...
    },
    {
      ...
      rows: [
        ...
        ['`cache_config`', '`Option<ContextCacheConfig>`', 'Opt-in [context caching](/docs/context-caching): Gemini caches the stable prefix server-side via explicit `cachedContents`, Anthropic maps it to a prompt-caching `cache_control` breakpoint, OpenAI-compatible endpoints ignore it.'],
      ],
    },
    {
      kind: 'api',
