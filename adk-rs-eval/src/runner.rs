...
use adk_rs_core::{InvocationContext, InvocationOrigin, RunConfig, SessionService, ToolContext};
use adk_rs_error::Result;
use adk_rs_genai_types::Part;
    ...
    pub async fn run_case(&self, set_id: &str, case: &EvalCase) -> Result<EvalResult> {
        ...
        for inv in &case.conversation {
            ...
            // Append the user content event.
            {
                ...
                s.events.push(adk_rs_core::Event::new(
                    "user",
                    adk_rs_core::LlmResponse {
            ...
            let mut actual_response = adk_rs_genai_types::Content::model_text("");
            let mut tool_uses: Vec<ToolUse> = Vec::new();
            let mut intermediate: Vec<adk_rs_genai_types::Content> = Vec::new();
...
mod tests {
    ...
    async fn eval_runs_response_match() {
        ...
        let runner = EvalRunner::new(
            ...
            "app",
            "u",
            vec![Arc::new(crate::metrics::ResponseMatch::new(0.5))],
        );
        let set = EvalSet {
            id: "s".into(),
            name: "demo".into(),
            eval_cases: vec![EvalCase {
                ...
                conversation: vec![Invocation {
                    user_content: adk_rs_genai_types::Content::user_text("hi"),
                    final_response: adk_rs_genai_types::Content::model_text("hello world"),
