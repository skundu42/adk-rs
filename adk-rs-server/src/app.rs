...
/// Bind and serve.
pub async fn serve(addr: SocketAddr, state: AppState) -> adk_rs_error::Result<()> {
    let app = build_router(state);
    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .map_err(|e| adk_rs_error::Error::other(format!("bind {addr}: {e}")))?;
    info!("adk-server listening on http://{addr}");
    axum::serve(listener, app)
        .await
        .map_err(|e| adk_rs_error::Error::other(format!("serve: {e}")))
}
