...
use adk_rs_genai_types::{Content, FunctionResponse, Part, Role};
            ...
            for _iter in 0..me.max_iterations {
                ...
                for fc in &calls {
                    let tool = req
                        ...
                        .cloned()
                        .ok_or_else(|| {
                            Error::from(adk_rs_error::ToolError::Unknown(fc.name.clone()))
                        })?;
...
fn response_to_event(author: &str, invocation_id: &str, resp: LlmResponse) -> Event {
    Event {
        ...
        timestamp: adk_rs_core::session::now_secs(),
        branch: None,
        response: resp,
...
mod tests {
    ...
    fn build_ctx(
        svc: Arc<dyn adk_rs_core::SessionService>,
        user_text: &str,
    ...
    async fn llm_agent_runs_single_turn() {
        ...
        let svc: Arc<dyn adk_rs_core::SessionService> = Arc::new(InMemorySessionService::new());
